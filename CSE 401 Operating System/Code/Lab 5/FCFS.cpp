...
int main()
{
    ...
    sort(processList, processList + number_of_process, comp);
    int current_time = 0;
    vector<process> process_order;

    for (i = 0; i < number_of_process; i++)
    {
        int wait = processList[i].arrival_time - current_time;
        if (wait > 0)
        {
            process temp;
            temp.process_id = "I";
            temp.completion_time = processList[i].arrival_time;
            temp.burst_time = wait;
            process_order.push_back(temp);
        }
        processList[i].starting_time = max(current_time, processList[i].arrival_time);
